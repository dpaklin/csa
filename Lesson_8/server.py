...
import logging
import select
import Lesson_8.logs.config_server_log
from Lesson_8.common.variables import DEFAULT_PORT, MAX_CONNECTIONS, ACTION, TIME, \
...
def process_message(message, names, listen_socks):
    ...
    if message[DESTINATION] in names and names[message[DESTINATION]] in listen_socks:
        ...
        for user in listen_socks:
            if user != names[message[SENDER]]:
                send_message(user, message)
        LOGGER.info(f'Отправлено сообщение всем пользователям'
                    f'от пользователя {message[SENDER]}.')
