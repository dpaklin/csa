...
import argparse
import logging
import Lesson_7.logs.config_client_log
...
def main():
    ...
    CLIENT_LOGGER.info(
        ...
        f'порт: {server_port}, режим работы: {client_mode}, имя клиента: {client_name}')

    # Инициализация сокета и сообщение серверу о нашем появлении
    try:
        transport = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        transport.connect((server_address, server_port))
        send_message(transport, create_presence(account_name=client_name))
        ...
        while True:
            ...
            if client_mode == 'send':
                try:
                    send_message(transport, create_message(transport, account_name=client_name))
